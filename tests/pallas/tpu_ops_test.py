...
class OpsTest(ptu.PallasTPUTest):
  ...
  def test_sublane_pad_high_in_element_window(self):
    if not jtu.is_cloud_tpu_at_least(year=2026, month=5, day=11):
      self.skipTest("Requires newer libtpu")
    ...
    non_overlap = 128
    overlap = 64
    tile_size = overlap + non_overlap
    num_tiles = n // non_overlap

    x = jnp.arange(n * d, dtype=jnp.float32).reshape(n, d)

    out_high = self.pallas_call(
        kernel,
        out_shape=jax.ShapeDtypeStruct((num_tiles, tile_size, d), jnp.float32),
        in_specs=[
            pl.BlockSpec(
                (pl.Element(tile_size, padding=(0, overlap)), pl.Element(d)),
                lambda i: (i * non_overlap, 0),
            )
        ],
        out_specs=pl.BlockSpec((None, tile_size, d), lambda i: (i, 0, 0)),
        grid=(num_tiles,),
    )(x)
    for i in range(num_tiles - 1):
      np.testing.assert_array_equal(
          out_high[i],
          x[i * non_overlap : i * non_overlap + tile_size],
      )
    np.testing.assert_array_equal(
        out_high[-1, :non_overlap, :], x[n - non_overlap :]
    )

  def test_sublane_pad_low_in_element_window(self):
    if not jtu.is_cloud_tpu_at_least(year=2026, month=5, day=11):
      self.skipTest("Requires newer libtpu")
    ...
    out_low = self.pallas_call(
        ...
        in_specs=[
            pl.BlockSpec(
                (pl.Element(tile_size, padding=(overlap, 0)), pl.Element(d)),
                lambda i: (i * non_overlap, 0),
            )
        ],
        out_specs=pl.BlockSpec((None, tile_size, d), lambda i: (i, 0, 0)),
    ...
    np.testing.assert_array_equal(out_low[0, overlap:, :], x[:non_overlap])
    for i in range(1, num_tiles):
      np.testing.assert_array_equal(
          out_low[i],
          x[i * non_overlap - overlap : i * non_overlap - overlap + tile_size],
    )
